...
입력된 단일 페이지 이미지 의료 기록을 분석하여 시각적 구조를 완벽히 보존한 마크다운(Markdown)으로 변환합니다.
...
4. 개인정보(PII) 처리 규칙:
    - 마스킹 대상: 주민등록번호, 주소, 전화번호만 반드시 `***`로 치환합니다.
    - 보존 대상: 환자 이름, 환자명, 나이, 성별, 병원명, 진료과, 의사 성명은 데이터의 맥락 파악을 위해 원문 그대로 유지합니다.
