...
- tags: 문서의 대분류 카테고리와 문서 affordance 성격의 단어를 5~8개 지정하세요. 어떤 범위의 원본 확인에 유용한지 드러나야 합니다. (예: 외래진료, 진료기록, 검사결과, 처방내역, 건강검진, 복합기록, 기간별기록)
- anchors: 검색어가 매칭될 수 있는 항목명과 문서 범위 키워드를 8~12개 지정하세요. 결과값이나 수치, 상세 용량이 포함되면 안 됩니다. (예: 혈압, 혈당, HbA1c, eGFR, 간기능, AST, ALT, CRP, 진료일, 문서일자)
- open_when: 정답을 요약하지 말고 이 원본을 열어볼 만한 넓은 확인 상황을 3~6개 명시하세요. 특정 결과값이 아니라 문서 유형, 날짜 범위, 기록 범위, 포함 정보군 기준으로 작성하세요.
