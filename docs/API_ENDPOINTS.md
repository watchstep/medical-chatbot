...
| `/upload/{token}` | 인증된 카카오 사용자가 받은 one-use 업로드 링크로 PDF/이미지 1개를 업로드 |
...
- `/upload/{token}`은 `/kakao/chat`에서 발급한 stateful one-use token만 허용한다.
- `/upload/{token}`은 Google Drive source를 만들지 않고 최근 active attachment 1개만 갱신한다.
- `/upload/{token}`은 PDF, JPEG, PNG, WebP 1개만 받으며 `MAX_UPLOAD_FILE_BYTES`를 초과하면 거절한다.
- 업로드 성공 후 같은 token으로 POST가 재전송되면, 같은 token으로 생성된 active attachment가 유효한 경우 완료 화면을 다시 반환하고 Gemini에 재업로드하지 않는다.
- 업로드 token URL, Gemini file_name, file_uri, 원본 파일명은 chat log와 관리자 대시보드에 노출하지 않는다.
...
- `/admin/cleanup-gemini-files`는 기존 Google Drive source runtime cache와 만료된 temporary attachment cache를 함께 정리한다.
